...
{
    struct bitstream * b = malloc( sizeof(struct bitstream) );
...
void put_bit(struct bitstream *b, Booleen bit)
{
    ...
    b->buffer=pose_bit(b->buffer,NB_BITS-b->nb_bits_dans_buffer-1,bit);
...
Booleen get_bit(struct bitstream *b)
{
    ...
    if(b->nb_bits_dans_buffer==0)
    {
        ...
        if (val<=255)
        {
            b->buffer=(unsigned char)val;
            b->nb_bits_dans_buffer=8;
        }
    }
    Booleen c= prend_bit(b->buffer,b->nb_bits_dans_buffer-1);
    b->nb_bits_dans_buffer--;
    return c; /* pour enlever un warning du compilateur */
}
